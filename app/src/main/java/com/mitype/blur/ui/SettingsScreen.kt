...
fun SettingsScreen(
    ...
    MiuixTheme {
        ...
        fun presetDesc(v: Int) = when (v) {
            Config.BLUR_LIGHT -> "混色×0.60 · 更通透，壁纸隐约可辨"
            Config.BLUR_HEAVY -> "混色×1.18 · 更浓郁奶霜，沉浸遮蔽"
            else -> "混色×1.00 · 原厂观感"
        }
            ...
            LazyColumn(
                ...
                if (connected && prefs != null) {
                    item {
                        Card(modifier = Modifier.padding(top = 12.dp).fillMaxWidth()) {
                            Text(
                                text = "ℹ️ 任何参数修改都需要重启输入法后生效：" +
                                        "系统设置 → 应用管理 → 小爱输入法 → 强制停止。",
                                fontSize = 13.sp,
                    ...
                    item {
                        Card(modifier = Modifier.padding(top = 12.dp).fillMaxWidth()) {
                            Text(
                                text = "⚠ 未连接 LSPosed 服务：请确认模块已启用后重新打开。",
                                fontSize = 14.sp,
                ...
                item {
                    ...
                    Card(modifier = Modifier.fillMaxWidth()) {
                        SwitchPreference(
                            checked = enable,
                            onCheckedChange = { enable = it; save { editor -> editor.putBoolean(Config.KEY_ENABLE, enable) } },
                            title = "启用毛玻璃",
                ...
                item {
                    Card(modifier = Modifier.padding(top = 8.dp, bottom = 24.dp).fillMaxWidth()) {
                        BasicComponent(
                            title = "重置为默认值",
                            summary = "柔光磨砂 / 材质锁深色 / 触感原生 / 清除开发者覆盖",
                            onClick = {
                                save { editor ->
